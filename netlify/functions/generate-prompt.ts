import { generateOptimizedPrompt, checkRateLimit } from '../../src/server/promptEngine';
import { PromptGenerationRequest } from '../../src/types';
...
export default async (req: Request) => {
  ...
  if (!checkRateLimit(clientIp)) {
    return new Response(
      JSON.stringify({
        error: 'Too many requests. Please wait a moment before generating another prompt.',
      }),
      {
        status: 429,
        headers: { 'Content-Type': 'application/json' },
  ...
  try {
    const body = (await req.json()) as PromptGenerationRequest;
    if (!body || !body.idea || typeof body.idea !== 'string') {
      return new Response(
        JSON.stringify({ error: 'Please enter an idea first.' }),
        {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    const result = await generateOptimizedPrompt(body);
    return new Response(JSON.stringify(result), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error: any) {
    console.error('Netlify function generate-prompt error:', error?.message || error);
